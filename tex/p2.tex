...
\textbf{Input:} State set $S$, box size d. \\
\textbf{Output:} Mesh size m.\\
\textbf{Initialize:} Empty hash table M. \\
 \For{s $\in$ S}{
...
\SetAlgoLined
%\KwResult{Mesh Table, Deterministic State Transition Matrix}
\textbf{Input:} State set $S$.\\
\textbf{Output:} Mesh M. \\
\textbf{Hyperparameters:} scaling factor f,  initial box size $d_{0}$. \\
\textbf{Initialize:} Empty list of mesh sizes H, empty list of d values D. \\
m = Size(CreateBoxMesh(S, $d_{0}$)) \\
d = $d_{0}$ \\ 
